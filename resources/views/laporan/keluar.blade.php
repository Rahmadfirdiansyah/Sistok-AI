...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .total-row td { border-top: 2px solid #ea580c; }
    ...
    <table class="data">
        <thead>
            <tr>
                ...
                <th style="width: 12%;">Dipakai Oleh</th>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td style="font-family: monospace; font-weight: bold; color: #ea580c;">{{ $t['id'] }}</td>
                <td>{{ $t['tanggal'] }}</td>
                <td>
                    <strong>{{ $t['barang'] }}</strong><br>
                    <span style="font-size: 7.5px; color: #9ca3af;">{{ $t['kode'] }}</span>
                </td>
                <td>{{ $t['dipakai_oleh'] }}</td>
                <td>{{ $t['tujuan'] }}</td>
                <td>{{ $t['lokasi'] }}</td>
                <td class="center" style="font-weight: bold; color: #ea580c;">-{{ $t['jumlah'] }} {{ $t['satuan'] }}</td>
                <td style="font-size: 8px; color: #6b7280;">{{ $t['keterangan'] }}</td>
            </tr>
            @empty
            <tr>
                <td colspan="9" style="text-align: center; padding: 20px; color: #9ca3af;">Tidak ada transaksi.</td>
            </tr>
            @endforelse
            @if(count($transaksis) > 0)
            <tr class="total-row">
                <td colspan="7" style="text-align: right;">TOTAL QTY KELUAR</td>
                <td class="center" style="color: #ea580c;">-{{ $totalQty }}</td>
                <td></td>
            </tr>
            @endif
        </tbody>
