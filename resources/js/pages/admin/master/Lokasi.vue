<template>
  <div class="space-y-4 md:space-y-5">
    ...
    <!-- MODAL DAFTAR BARANG PER LOKASI -->
      ...
      <div class="space-y-4">
        ...
        <div class="flex flex-col sm:flex-row items-center justify-between gap-3">
          <div class="relative w-full sm:w-64">
            ...
            <input v-model="itemSearch" type="text" placeholder="Cari barang di lokasi ini..."
          ...
          <button @click="goToDataBarang(selectedLokasi?.nama)"
...
export default {
  ...
  computed: {
    ...
    itemsModalTitle() {
      if (!this.selectedLokasi) return 'Daftar Barang'
      const total = this.locationBarangs.length
      return `📍 Daftar Barang — Lokasi: ${this.selectedLokasi.nama} (${total} Item)`
    },
    locationBarangs() {
      if (!this.selectedLokasi) return []
      return this.allBarangs.filter(b => b.lokasi === this.selectedLokasi.nama || b.lokasi_id === this.selectedLokasi.id)
    },
    filteredModalItems() {
      const q = this.itemSearch.toLowerCase().trim()
      return this.locationBarangs.filter(b => !q || b.nama.toLowerCase().includes(q) || b.kode.toLowerCase().includes(q) || (b.kategori || '').toLowerCase().includes(q))
    },
  },
  mounted() {
    this.fetchLokasis()
  ...
  methods: {
    ...
    async openItemsModal(l) {
      this.selectedLokasi = l
      this.itemSearch = ''
    ...
    goToDataBarang(name) {
      ...
      this.$router.push({ name: 'admin.barang', query: { lokasi: name } })
    },
