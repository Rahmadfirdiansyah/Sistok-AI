<template>
  <div class="space-y-4 md:space-y-5">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 sm:gap-0">
      <div>
        <h1 class="text-xl font-bold text-gray-800 m-0">Riwayat Transaksi</h1>
        <p class="text-gray-400 text-sm mt-1 m-0">Riwayat gabungan untuk barang masuk dan barang keluar.</p>
      </div>
    ...
    <div class="flex flex-col xl:flex-row gap-3">
      <div class="relative flex-grow">
        ...
        <input v-model="searchInput" type="text" placeholder="Cari nama barang, kategori, ID..."
      ...
      <div class="grid grid-cols-12 md:flex md:flex-nowrap items-center gap-2">
        ...
        <select v-model="filterJenis" class="col-span-6 md:w-auto px-3 py-2.5 rounded-xl border border-gray-200 text-sm text-gray-600 outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 transition-all bg-white">
          <option value="">Semua Transaksi</option>
          <option value="masuk">Barang Masuk</option>
          <option value="keluar">Barang Keluar</option>
        </select>

        <select v-model="filterKategori" class="col-span-6 md:w-auto px-3 py-2.5 rounded-xl border border-gray-200 text-sm text-gray-600 outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 transition-all bg-white">
        ...
        <button @click="resetFilter" class="col-span-2 md:w-auto flex items-center justify-center px-3 py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-600 text-sm font-semibold rounded-xl transition-all border-0 cursor-pointer" title="Reset Filter">
    ...
    <div class="bg-white rounded-2xl border border-gray-100 overflow-hidden relative min-h-[300px]">
      ...
      <div class="md:hidden divide-y divide-gray-100">
        <div v-if="paginated.length === 0 && !loading" class="text-center py-12 text-gray-400 text-sm">
          Tidak ada transaksi ditemukan.
        </div>
        <div v-for="t in paginated" :key="t.id + '_mobile'" class="p-4 hover:bg-gray-50 transition-all">
          <div class="flex items-center justify-between mb-2">
            <span class="text-[11.5px] font-mono font-semibold px-2 py-0.5 rounded-lg"
              :class="t.jenis === 'masuk' ? 'text-green-600 bg-green-50' : 'text-orange-500 bg-orange-50'">
              {{ t.id }}
            </span>
            <span class="inline-flex items-center gap-1 font-bold px-2 py-0.5 rounded-full text-[10px] uppercase tracking-wide"
              :class="t.jenis === 'masuk' ? 'text-green-600 bg-green-50' : 'text-orange-500 bg-orange-50'">
              {{ t.jenis }}
            </span>
          </div>
          ...
          <div class="flex items-center justify-between mb-1">
            ...
            <span class="font-bold text-[13px] whitespace-nowrap"
              :class="t.jenis === 'masuk' ? 'text-green-600' : 'text-orange-500'">
              {{ t.jenis === 'masuk' ? '+' : '-' }}{{ t.qty }} {{ t.satuan }}
            </span>
          </div>
        ...
        <table class="w-full text-sm">
          ...
          <tbody class="divide-y divide-gray-50">
            ...
            <tr v-for="t in paginated" :key="t.id" class="hover:bg-gray-50 transition-all">
              ...
              <td class="px-4 py-3.5 text-gray-600 text-[12.5px] whitespace-nowrap">{{ t.tanggal }}</td>
              <td class="px-4 py-3.5">
                <p class="font-medium text-gray-700 m-0">{{ t.barang }}</p>
                <p class="text-[11px] text-gray-400 m-0">{{ t.kategori }}</p>
              </td>
              <td class="px-4 py-3.5 text-gray-500 text-[12.5px]">{{ t.lokasi }}</td>
              <td class="px-4 py-3.5 text-center font-bold text-[13px] whitespace-nowrap"
                :class="t.jenis === 'masuk' ? 'text-green-600' : 'text-orange-500'">
                {{ t.jenis === 'masuk' ? '+' : '-' }}{{ t.qty }} {{ t.satuan }}
              </td>
              <td class="px-4 py-3.5 text-gray-400 text-[12.5px] max-w-[200px]">
...
export default {
  ...
  computed: {
    filtered() {
      // Frontend search filter only (since date, jenis, lokasi, kategori are filtered on backend)
      return this.transaksis.filter(t => {
        const q = this.search.toLowerCase()
        if (!q) return true
        
        return t.barang.toLowerCase().includes(q) 
            || t.kategori.toLowerCase().includes(q) 
            || t.id.toLowerCase().includes(q)
            || t.oleh_tujuan.toLowerCase().includes(q)
      })
    },
  ...
  watch: {
    ...
    search()          { this.page = 1 },
    filterStartDate() { this.fetchData() },
    filterEndDate()   { this.fetchData() },
  ...
  methods: {
    async fetchOptions() {
      try {
        const [resL, resK] = await Promise.all([
          api.get('/lokasis'),
          api.get('/kategoris'),
        ])
    ...
    async fetchData() {
      // Create cache key based on current filter parameters so different filters are cached separately
      ...
      if (this.filterJenis) params.append('jenis', this.filterJenis)
      
      const cacheKey = 'riwayat_transaksi_' + params.toString()
      ...
      this.page = 1
      try {
        const response = await api.get(`/laporan/transaksi?${params.toString()}`)
        this.transaksis = response.data
        setCache(cacheKey, { transaksis: this.transaksis })
      } catch (error) {
    ...
    resetFilter() {
      ...
      this.searchInput = ''
      this.search = ''
      // fetchData() will be triggered by watchers
    },
    printLaporan() {
      const params = new URLSearchParams();
      if (this.filterStartDate) params.append('start_date', this.filterStartDate)
      ...
      const url = `/api/laporan/transaksi/pdf?${params.toString()}`;
    ...
    exportExcel() {
      ...
      const url = `/api/laporan/transaksi/excel?${params.toString()}`;
      window.location.href = url;
    },
    showDetail(t) {
      let qtyClass = t.jenis === 'masuk' ? 'text-green-600' : 'text-orange-500';
      let qtyPrefix = t.jenis === 'masuk' ? '+' : '-';
      let idPrefix = t.jenis === 'masuk' ? 'TRX-IN-' : 'TRX-OUT-';
      
      let detailHtml = `
        <div class="text-left space-y-3 mt-4 text-sm text-gray-700">
          <div class="flex justify-between border-b pb-2">
            <span class="font-semibold text-gray-500">ID Transaksi</span>
            <span class="font-mono text-indigo-600">${idPrefix}${String(t.id).padStart(3, '0')}</span>
          </div>
          <div class="flex justify-between border-b pb-2">
            <span class="font-semibold text-gray-500">Jenis</span>
            <span class="uppercase font-bold ${qtyClass}">${t.jenis}</span>
          </div>
          <div class="flex justify-between border-b pb-2">
          ...
          <div class="flex justify-between border-b pb-2">
            <span class="font-semibold text-gray-500">Pemakai/Tujuan</span>
            <span>${t.oleh_tujuan}</span>
          </div>
          <div class="flex justify-between border-b pb-2">
