<template>
  <div class="space-y-4 md:space-y-5">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 sm:gap-0">
      <div>
        <h1 class="text-xl font-bold text-gray-800 m-0">Laporan Barang Masuk</h1>
        <p class="text-gray-400 text-sm mt-1 m-0">Riwayat transaksi penerimaan barang masuk ke gudang.</p>
      </div>
    ...
    <div class="flex flex-col sm:flex-row gap-3">
      <div class="relative flex-grow">
        ...
        <input v-model="searchInput" type="text" placeholder="Cari nama barang atau supplier..."
      ...
      <div class="md:hidden divide-y divide-gray-100">
        ...
        <div v-for="t in paginated" :key="t.id + '_mobile'" class="p-4 hover:bg-gray-50 transition-all">
          <div class="flex items-center justify-between mb-2">
            <span class="text-[11.5px] font-mono font-semibold text-indigo-500 bg-indigo-50 px-2 py-0.5 rounded-lg">TRX-IN-{{ String(t.id).padStart(3, '0') }}</span>
          </div>
          ...
          <div class="flex items-center justify-between mb-1">
            ...
            <span class="font-bold text-[13px] text-green-600 whitespace-nowrap">
              +{{ t.qty }} {{ t.satuan }}
            </span>
          </div>
          ...
          <div class="text-xs text-gray-500 space-y-1 mb-3">
            ...
            <div class="flex items-center gap-1.5 truncate">
              ...
              <span class="truncate">{{ t.supplier }}</span>
            </div>
          </div>
        ...
        <table class="w-full text-sm">
          <thead>
            <tr class="bg-gray-50 border-b border-gray-100">
              ...
              <th class="text-left px-4 py-3.5 text-[11px] font-bold text-gray-400 uppercase tracking-wide">Supplier</th>
              <th class="text-left px-4 py-3.5 text-[11px] font-bold text-gray-400 uppercase tracking-wide">Lokasi Masuk</th>
          ...
          <tbody class="divide-y divide-gray-50">
            ...
            <tr v-for="t in paginated" :key="t.id" class="hover:bg-gray-50 transition-all">
              ...
              <td class="px-4 py-3.5 text-gray-500 text-[12.5px]">{{ t.supplier }}</td>
              <td class="px-4 py-3.5 text-gray-500 text-[12.5px]">{{ t.lokasi }}</td>
              <td class="px-4 py-3.5 text-right font-bold text-green-600 text-[13px]">
                +{{ t.qty }} {{ t.satuan }}
              </td>
...
export default {
  ...
  computed: {
    filtered() {
      return this.transaksis.filter(t => {
        ...
        const matchQ = !q || t.barang.toLowerCase().includes(q) || t.supplier.toLowerCase().includes(q) || t.kodeBarang.toLowerCase().includes(q)
  ...
  methods: {
    async fetchData() {
      const cached = getCache('laporan_masuk')
      if (cached) {
      ...
      try {
        ...
        setCache('laporan_masuk', { transaksis: this.transaksis, lokasis: this.lokasis })
      } catch (error) {
    ...
    printLaporan() {
      ...
      const url = `/api/laporan/masuk/pdf?${params.toString()}`;
    ...
    exportExcel() {
      ...
      const token = localStorage.getItem('token');
      if (token) params.append('token', token);
      const url = `/api/laporan/masuk/excel?${params.toString()}`;
      ...
      let detailHtml = `
        <div class="text-left space-y-3 mt-4 text-sm text-gray-700">
          <div class="flex justify-between border-b pb-2">
            ...
            <span class="font-mono text-indigo-600">TRX-IN-${String(t.id).padStart(3, '0')}</span>
          </div>
          <div class="flex justify-between border-b pb-2">
          ...
          <div class="flex justify-between border-b pb-2">
            <span class="font-semibold text-gray-500">Supplier</span>
            <span>${t.supplier}</span>
          </div>
          <div class="flex justify-between border-b pb-2">
            <span class="font-semibold text-gray-500">Lokasi Masuk</span>
            <span>${t.lokasi}</span>
          </div>
          <div class="flex justify-between border-b pb-2">
            <span class="font-semibold text-gray-500">Jumlah</span>
            <span class="text-green-600 font-bold">+${t.qty} ${t.satuan}</span>
          </div>
          <div class="pt-2">
